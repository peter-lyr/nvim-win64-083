...
local main = function()
  ...
  for _, line in ipairs(lines) do
    local cells = f['split'](f['trim'](line), '|')
    if cols < #cells then
      cols = #cells
    end
  end
  ...
  for _, line in ipairs(lines) do
    ...
    for i=1, cols do
      ...
      table.insert(matrix, {f['strlen'](cell), f['strwidth'](cell)})
    end
  ...
  f['setline'](M.markdowntable_line, newLines)
  c(string.format([[norm %dgg0%d|]], ll[2], ll[3]))
end
